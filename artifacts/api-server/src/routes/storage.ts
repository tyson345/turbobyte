 ...
 *
 * Request a presigned PUT URL for a new upload slot.
 * The client sends JSON metadata (name, size, contentType) — NOT the file.
 * Then uploads the file directly to the returned presigned URL.
 * Admin-gated so public callers cannot mint write-capable URLs.
 */
router.post(
 ...
 * GET /storage/objects/*
 *
 * Serve object entities from private S3 bucket.
 *
 * Access rules:
 *   - Published project media (thumbnail / gallery images referenced in DB) → public
 *   - Everything else (drafts, orphaned uploads, resumes) → admin-only
 */
router.get('/storage/objects/*path', async (req: Request, res: Response) => {
  try {
    ...
    const response = await objectStorageService.downloadObject(objectHandle);
